...
4. **Fill in the form:**
   ...
   - **API address / Base URL** — paste the service gateway address shown by the tool **exactly as displayed**. It differs per tool: Codex uses the `…/v1` address, Claude Code the bare domain without `/v1` (it appends `/v1/messages` itself). Do not add or remove a trailing slash or `/v1`; the address rules are explained under fault B.
   ...
   - **Model** — enter the value suggested by the tool (the suggestion is per tool: a `gpt-…` model for Codex, a `claude-…` model for Claude Code), or the model name assigned to you. This field may differ per person.
5. **Save.**

# Configure the company gateway in CC Switch
...
4. **Fill in the form:**
   - **Name** — anything you like; the name suggested by the tool (for example "Company Gateway") is easiest to recognise later.
   - **API address / Base URL** — paste the company gateway address shown by the tool **exactly as displayed**. Do not add or remove a trailing slash or `/v1`; the address rules are explained under fault B.
   - **Protocol / request format (Codex only)** — choose **Responses**. This is the protocol the company gateway supports and recommends. If the form has no such option, the current version already defaults to Responses.

...
4. **填写表单：**
   ...
   - **API 地址 / Base URL**：粘贴工具显示的服务网关地址。两个工具的地址不同：Codex 用带 `/v1` 的地址，Claude Code 用不带 `/v1` 的域名（它会自己补 `/v1/messages`）。请**原样粘贴**，不要自己加或去掉结尾的斜杠、`/v1`；地址处理规则见故障 B。
   ...
   - **模型**：填写工具给出的建议值（按工具分别给出：Codex 是 `gpt-…`，Claude Code 是 `claude-…`），或者你被分配的模型名。这个字段每个人可能不同。
5. **保存。**

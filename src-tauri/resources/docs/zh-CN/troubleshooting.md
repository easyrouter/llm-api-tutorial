...
网关能连上，但路径不存在。地址处理规则（两个工具不一样）：

- 结尾的 `/` 会被自动去掉；
- **Codex**：只填域名（如 `https://gateway.example.com`）时会自动补上 `/v1`，已带 `/v1` 的地址原样使用；
- **Claude Code**：地址就是域名本身，不加 `/v1`——它会自己在后面补 `/v1/messages`。所以带 `/v1` 的地址会变成 `…/v1/v1/messages`，恰好返回这个 404，请把 `/v1` 去掉；
- 结尾加 `#` 表示"按字面使用，别再追加任何路径"——只有网关明确要求时才用。

最稳妥的做法：回到本工具"配置"页，把显示的地址**原样复制**过去——每个页签显示的都是该工具自己的地址；那里的地址预览会显示最终生效的完整地址。

...
| `guide`     | M3  | `build_guide`, `validate_api_key`, `preview_url`, `codex_config_template`                                                                         | pure functions, unit-tested; step codes = step ids (`guide:<id>.title` / `guide:<id>.body`); `GuideStep.branch` (`chatgpt_login` / `api_key` / null) selects the Codex account path; template carries `<API-KEY>` placeholder, substituted by the UI at apply/copy time                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |

...
```bash
git clone <repo> && cd codex-onboarding
...
- Config override for local testing without touching the bundled preset:
  put an `app-config.json` in the app config dir (`%APPDATA%/com.company.codex-onboarding/` on
  Windows, `~/Library/Application Support/com.company.codex-onboarding/` on macOS).
